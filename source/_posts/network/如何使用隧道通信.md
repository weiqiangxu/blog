...
ip netns exec a ip addr add 10.1.1.5/24 dev veth2
ip netns exec a ip link set veth2 up
ip netns exec a ip route add default via 10.1.1.1
ip netns exec b ip addr add 10.1.1.7/24 dev veth4
ip netns exec b ip link set veth4 up
ip netns exec b ip route add default via 10.1.1.1
brctl addbr br-link
...
``` bash
ip netns exec a ip link delete vxlan0
ip netns exec a ip link add vxlan0 type vxlan \
    id 42 \
    dstport 4789 \
    remote 10.1.1.7 \
    local 10.1.1.5 \
    dev veth2
ip netns exec a ip -d link 
ip netns exec a ip addr add 172.1.1.2/24 dev vxlan0
ip netns exec a ip link set vxlan0 up
ip netns exec a ip route
ip netns exec a bridge fdb
ip netns exec b ip link delete vxlan0
ip netns exec b ip link add vxlan0 type vxlan \
    id 42 \
    dstport 4789 \
    remote 10.1.1.5 \
    local 10.1.1.7 \
    dev veth4
ip netns exec b ip -d link 
ip netns exec b ip addr add 172.1.1.3/24 dev vxlan0
ip netns exec b ip link set vxlan0 up
ip netns exec b ip route
ip netns exec b bridge fdb
ip netns exec a ping 172.1.1.3
```
